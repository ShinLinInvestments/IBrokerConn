...
def check_path_prefix(path_prefix:str):
    ...
    if not os.path.exists(path_last_dir):
        ...
        xlog.info(path_prefix + " does not exist, so making it...")
    return
...
def write_csv(df:pd.DataFrame, path_prefix, col_datetime, file_freq:int = 8, index = False, num_cores = xutils_num_cores):
    if len(df) == 0: return
    ...
    mpool = mc.Pool(processes = num_cores)
    mpool.map(lambda x : _write_single_csv(df, (all_dtss == x), path_prefix + x + '.csv', index=index), all_dts)
